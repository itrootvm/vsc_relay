...
- Storing app secrets in the macOS Keychain, or in a permission-restricted `relay.env` on Linux and Windows.
...
- macOS / Linux: Unix domain sockets under `~/.vsc-relay` (owner-only `0700` directory),
  isolated by filesystem permissions.
